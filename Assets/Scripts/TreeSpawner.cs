...
public class TreeSpawner : MonoBehaviour
{
    ...
    public void Spawn(TerrainData[,] dataMap)
    {
        ...
        for( int x =0; x < size; x++ )
        {
            ...
            {
                if (dataMap[x, z].biome.spanwPrefabs)
                {
    ...
    void SpawnTree(Vector3 position, GameObject[] availablePrefabs)
    {
        ...
        if (!Physics.Raycast(new Ray(truePos, Vector3.down), out RaycastHit hit))
            return;

        var treeObj = Instantiate(availablePrefabs[Random.Range(0, availablePrefabs.Length)], truePos, Quaternion.identity);
        treeObj.transform.position = new Vector3(hit.point.x, hit.point.y - 0.02f, hit.point.z);
    }
}
